...
- Filters to 750 pre-curated breast cancer biomarker genes
- Log2(TPM + 1) normalizes and filters low-expression genes
- Ranks genes by differential expression p-value (Mann-Whitney U)
- Saves clean output files for teammates to use

OUTPUT FILES (saved to your project folder):
- processed_cancer_tpm.csv       → log2-normalized cancer TPM (96 samples)
- processed_normal_tpm.csv       → log2-normalized normal TPM (32 samples)
- task1_feature_matrix.csv       → combined cancer+normal matrix for Task 1
- task1_labels.csv               → cancer(1) / normal(0) labels for Task 1
- task2_feature_matrix.csv       → cancer-only matrix for Task 2
- task2_labels.csv               → recurrent(1) / non-recurrent(0) for Task 2
- ranked_biomarker_genes.csv     → 740 genes ranked by DE p-value
- preprocessing_summary.txt      → summary of what was done (for paper methods)

HOW TO RUN:
1. Make sure all data files are in your BENG 203 project folder
...
# ─────────────────────────────────────────────────────────────
# STEP 2: LOG2 NORMALIZATION + LOW-EXPRESSION FILTER
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# STEP 3: FILTER TO BIOMARKER GENES
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# STEP 4: RANK GENES BY DIFFERENTIAL EXPRESSION (Mann-Whitney U)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# STEP 7: WRITE METHODS SUMMARY (for paper)
# ─────────────────────────────────────────────────────────────
summary = f"""PREPROCESSING SUMMARY — BENG203 Project
Generated by: script1_preprocessing.py
=========================================
...
print(f"  {OUT}")
print("=" * 60)
print("\nNext step: share these CSV files with your teammates")
print("They load them with: pd.read_csv('task1_feature_matrix.csv', index_col=0)")
